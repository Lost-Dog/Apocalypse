...
namespace MxM
{
    ...
    *********************************************************************************************/
    ...
    public partial class MxMAnimator : MonoBehaviour
    {
        ...
        private bool m_loggedMissingClipReference = false;
       ...
       // private static int s_mxmAnimatorStack;              //The number of MxMAnimator's currently waiting to be scheduled
        ...
        public float ChosenAnimationNormalizedTime
        {
            get
            {
                if (TryGetAnimClip(ChosenPose.AnimId, out AnimationClip chosenClip)
                    && chosenClip.length > Mathf.Epsilon)
                {
                    return ChosenAnimationTime / chosenClip.length;
                }
    ...
    protected virtual void OnEnable()
        ...
        *********************************************************************************************/
        private void SetupAnimator()
        {
            ...
            else //Blending Setup
            {
                ...
                if (!TryGetAnimClip(m_chosenPose.PrimaryClipId, out AnimationClip clip))
                {
                    return;
                }
        ...
        *********************************************************************************************/
        public ScriptPlayable<MotionMatchingPlayable> CreateMotionMatchingPlayable(ref PlayableGraph a_playableGraph)
        {
            ...
            if (!TryGetAnimClip(m_chosenPose.PrimaryClipId, out AnimationClip clip))
            {
                return ScriptPlayable<MotionMatchingPlayable>.Null;
            }
        ...
        *********************************************************************************************/
        public void MxMLateUpdate()
        {
            ...
            {
                // Safety: complete pose/traj jobs even if Phase2 was skipped (e.g. animator
                // was paused during PrepareFrame).  Completing an already-completed handle is a no-op.
                m_poseJobHandle.Complete();
                m_trajJobHandle.Complete();
        ...
        *********************************************************************************************/
        private void UpdateMatching_Phase1()
        {
            ...
            if (!TryGetAnimClip(m_chosenPose.PrimaryClipId, out AnimationClip curClip)
                || curClip.length <= Mathf.Epsilon)
            {
                return;
            }
        ...
        *********************************************************************************************/
        private void FinalizePoseSearch(int a_chosenPoseId)
        {
            ...
            bool currentPoseLoops = TryGetAnimClip(m_curInterpolatedPose.PrimaryClipId, out AnimationClip curPoseClip)
                && curPoseClip.isLooping;
            ...
            {
                bool chosenPoseLoops = TryGetAnimClip(m_chosenPose.PrimaryClipId, out AnimationClip chosenPoseClip)
                    && chosenPoseClip.isLooping;
        ...
        *********************************************************************************************/
        private void ComputeCurrentPose()
        {
            ...
            if (m_chosenPose.AnimType != EMxMAnimtype.Composite)
            {
                if (!TryGetAnimClip(m_chosenPose.PrimaryClipId, out AnimationClip chosenClip))
                {
                    return;
                }
                ...
                if (chosenClipLength <= Mathf.Epsilon)
                {
                    return;
                }
            }
            ...
            {
                if (!TryGetAnimClip(m_dominantPose.PrimaryClipId, out AnimationClip dominantClip))
                {
                    return;
                }
                ...
                if (dominantClipLength <= Mathf.Epsilon)
                {
                    return;
                }
            }
...
//#endif
        ...
        private bool TryGetAnimClip(int clipId, out AnimationClip clip)
        {
            clip = null;

            if (CurrentAnimData == null || CurrentAnimData.Clips == null
                || clipId < 0 || clipId >= CurrentAnimData.Clips.Length)
            {
                ReportMissingClipReference(clipId);
                return false;
            }

            clip = CurrentAnimData.Clips[clipId];
        ...
        private void ReportMissingClipReference(int clipId)
        {
            if (m_loggedMissingClipReference)
            {
                return;
            }

            m_loggedMissingClipReference = true;
            Debug.LogWarning($"MxMAnimator on '{name}' found a missing or invalid clip reference (clip id {clipId}) in MxMAnimData. Rebuild or reassign AnimData clips.", this);
        }
        ...
        *********************************************************************************************/
        public void Pause()
        {
            if (!IsPaused)
            {
                if (p_trajectoryGenerator != null)
                    p_trajectoryGenerator.Pause();

                IsPaused = true;

                if (m_animMixerConnected && m_animationMixer.IsValid())
        ...
        *********************************************************************************************/
        ...
        {
            if (!TryGetAnimClip(m_dominantPose.PrimaryClipId, out AnimationClip clip))
            {
                return null;
            }
