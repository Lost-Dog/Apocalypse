...
namespace MxM
{
    ...
    public class MxMBlendSpace : ScriptableObject, IMxMAnim
    {
        ...
        public void ValidateBaseData()
        {
            ...
            // Strip null entries before using any clip reference so destroyed/missing
            // clips don't throw MissingReferenceException during validation.
            if (m_clips != null)
            {
                for (int i = m_clips.Count - 1; i >= 0; --i)
                {
                    ...
                    {
                        m_clips.RemoveAt(i);
                        if (m_positions != null && i < m_positions.Count)
