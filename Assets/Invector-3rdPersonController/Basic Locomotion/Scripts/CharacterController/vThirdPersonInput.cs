...
namespace Invector.vCharacterController
{
    ...
    public class vThirdPersonInput : vMonoBehaviour, vIAnimatorMoveReceiver
    {
        ...
        {
            // When an external bridge has permanently locked legacy input (lockInput == true
            // was set externally and bridgeLockedInput is active), never allow Invector
            // systems to clear it — doing so re-enables legacy Input polling that throws
            // InvalidOperationException under the new Input System package.
            if (!value && _bridgeLockedInput) return;
            lockInput = value;
            if (value)
            {
        ...
        /// <summary>
        /// Called by an external input bridge to permanently lock legacy input polling.
        /// Once set, SetLockBasicInput(false) becomes a no-op so Invector systems cannot
        /// accidentally re-enable legacy input (e.g. ExitCover → SetLockAllInput(false)).
        /// </summary>
        public void SetBridgeLockInput(bool value)
        {
            _bridgeLockedInput = value;
            lockInput = value;
        }

        private bool _bridgeLockedInput;
        ...
        public virtual void MoveInput()
        {
            ...
            if (!lockInput && Input.GetKeyDown(toggleWalk))
            {
