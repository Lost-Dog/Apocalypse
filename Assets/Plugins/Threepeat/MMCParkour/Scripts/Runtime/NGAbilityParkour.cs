...
namespace Threepeat
{
    public class NGAbilityParkour : MonoBehaviour
    {
        ...
        private void Initialize()
        {
            ...
            {
                Debug.LogWarning("NGAbilityParkour Initialize skipped IK/event setup because config or config.capabilities is null.");
                return;
            }
        ...
        {
            if (eventIKMgr == null || config == null || config.capabilities == null)
            {
                if (debugMode) Debug.LogWarning("NGAbilityParkour LoadIKEvents skipped because eventIKMgr or config is null.");
                return;
            }
            ...
            foreach (NGParkourSettings.ParkourCapability cape in config.capabilities)
            {
                ...
                if (cape.runningEvent != null && cape.runningEvent.animationClipInfos != null)
                {
                ...
                if (cape.standingEvent != null && cape.standingEvent.animationClipInfos != null)
                {
        ...
        private void EnsureMxMReadyForParkour()
        {
            // If GC2 temporarily owns locomotion, parkour checks/actions can silently fail.
            MMCGameCreator2 bridge = GetComponent<MMCGameCreator2>();
            if (bridge != null && (!bridge.MMLCCurrentlyEnabled || !bridge.IsMxMContributing()))
            {
                bridge.SetMxMAnimatorBlendWeight(1f, 0f, false);
            }
            ...
            if (controllerWrapper != null && !controllerWrapper.enabled)
            {
                controllerWrapper.enabled = true;
            }
        }

        public bool ParkourAction_CanPerformAction(ContextualActionTrigger.TriggeringCondition trigger)
        {
            EnsureMxMReadyForParkour();
