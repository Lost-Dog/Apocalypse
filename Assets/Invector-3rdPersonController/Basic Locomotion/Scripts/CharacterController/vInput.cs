...
#else
            // New Input System device detection
            DetectInputDeviceNewInputSystem();
#endif
        }

        private void DetectInputDeviceNewInputSystem()
        {
            ...
            switch (inputDevice)
            {
                case InputDevice.MouseKeyboard:
                    ...
                    break;
                case InputDevice.Joystick:
                    if (mouseKeyboardActive && !gamepadActive)
                    {
                        inputDevice = InputDevice.MouseKeyboard;
                        if (hud != null)
                        {
        ...
        private static bool IsGamepadActive()
        {
            ...
            foreach (var control in gamepad.allControls)
            {
                if (control is ButtonControl btn && btn.wasPressedThisFrame)
                    return true;
                if (control is AxisControl axis && Mathf.Abs(axis.ReadValue()) > 0.25f)
                    return true;
            }
            return false;
        }

        private static bool IsMouseKeyboardActive()
        {
...
#else
            // Legacy input is not available; always report buttons unavailable so
            // GenericInput polling is a clean no-op. The InvectorInputBridge drives
            // all input via the new Input System instead.
            return false;
#endif
        }
