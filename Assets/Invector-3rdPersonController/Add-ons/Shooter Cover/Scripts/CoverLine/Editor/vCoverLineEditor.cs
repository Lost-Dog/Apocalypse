        ...
        {           
            ...
            if (point.conectedLeft && point.curveType!= vCoverLine.CurveType.Linear && point.curveType!= vCoverLine.CurveType.LinearLeft)
            {
                ...
                if (e.shift || selectionState == SelectionState.Left && currentPoint == point)
                {
                    ...
                    if ((e.shift && !e.control))
                        { var fmh_87_75_639139444271408955 = Quaternion.identity; tangentLeft = Handles.FreeMoveHandle(tangentLeft, HandleUtility.GetHandleSize(tangentLeft) * 0.15f, Vector3.zero, Handles.RectangleHandleCap); }
                    else
                    {
            ...
            if (point.conectedRight && point.curveType != vCoverLine.CurveType.Linear && point.curveType != vCoverLine.CurveType.LinearRight)
            {
                ...
                if ((e.shift && !e.control) || selectionState == SelectionState.Right && currentPoint == point)
                {
                    EditorGUI.BeginChangeCheck();
                    if ((e.shift && !e.control))
                        { var fmh_121_77_639139444271419793 = Quaternion.identity; tangentRight = Handles.FreeMoveHandle(tangentRight, HandleUtility.GetHandleSize(tangentRight) * 0.15f, Vector3.zero, Handles.RectangleHandleCap); }
                    else
                    {
            ...
            if ((e.shift && !e.control)  || selectionState == SelectionState.Center && currentPoint == point)
            {
                ...
                if ((e.shift && !e.control))
                {
                    var fmh_179_65_639139444271423026 = Quaternion.identity; position = Handles.FreeMoveHandle(position, HandleUtility.GetHandleSize(position) * 0.15f, Vector3.zero, Handles.RectangleHandleCap);
                }
