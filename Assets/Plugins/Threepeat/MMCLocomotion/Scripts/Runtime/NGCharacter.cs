...
namespace Threepeat {
    ...
    public class NGCharacter : MonoBehaviour
    {
        ...
        private Coroutine deferredInitializationRoutine = null;
        private const float DEFERRED_TAG_INIT_TIMEOUT_SECONDS = 5f;
        ...
        public void InitializeBehaviors(bool forceReassign = false)
        {
            ...
            if (mxmAnimator == null)
            {
                Debug.LogErrorFormat("No active MxMAnimator found for character( {0} ).", gameObject.name);
                return;
            }
            ...
            if (mxmTrajectoryGenerator == null)
            {
                Debug.LogErrorFormat("MxMTrajectoryGenerator missing on character( {0} ) MxMAnimator object.", gameObject.name);
                return;
            }
            ...
            if (!TryFinalizeTagInitialization())
            {
                StartDeferredTagInitialization();
                return;
            }
        }

        private bool TryFinalizeTagInitialization()
        {
            if (mxmAnimator == null)
            {
                return false;
        ...
        private void StartDeferredTagInitialization()
        {
            ...
            {
                deferredInitializationRoutine = StartCoroutine(DeferredTagInitializationRoutine());
            }
        }

        private IEnumerator DeferredTagInitializationRoutine()
        {
            float deadline = Time.realtimeSinceStartup + DEFERRED_TAG_INIT_TIMEOUT_SECONDS;
