...
{
    [Title("Instantiate Multiple with Pattern and Delay")]
    [Description("Creates multiple instances of a referenced game object using various patterns (Circle, Line, Grid, Random, Wave, Spiral, Heart, Star) with a delay between each spawn")]

    [Category("KingEdward/Skill Tree/VFX/Instantiate Multiple with Pattern and Delay")]
    ...
    [Parameter("Time Between Spawns", "Delay in seconds between each instantiation")]
    
    [Image(typeof(IconCubeSolid), ColorTheme.Type.Blue, typeof(OverlayPlus))]
    
    [Serializable]
    public class InstructionGameObjectInstantiateMultiplePatternWithDelay : Instruction
    {
        ...
        [SerializeField]
        private float m_SpiralHeight = 1f;

        [SerializeField]
        private int m_StarPoints = 5;

        [SerializeField]
        private float m_StarInnerRadius = 0.5f;

        [SerializeField]
        private float m_TimeBetweenSpawns = 0.2f;

        public override string Title => $"Instantiate Multiple {this.m_GameObject} in {this.m_Pattern} (Delay)";
        ...
        protected override async Task Run(Args args)
        {
            ...
            for (int i = 0; i < positions.Count; i++)
            {
                ...
                if (i < positions.Count - 1 && m_TimeBetweenSpawns > 0f)
                {
                    await Task.Delay((int)(m_TimeBetweenSpawns * 1000f));
                }
            }
