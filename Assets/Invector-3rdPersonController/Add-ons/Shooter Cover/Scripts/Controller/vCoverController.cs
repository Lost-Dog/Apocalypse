...
namespace Invector.vCover
{
    ...
    public class vCoverController : vMonoBehaviour
    {
        ...
        {
            // When enterExitInput.useInput is false an external bridge owns all cover
            // enter/exit decisions. Skip GenericInput polling entirely to prevent
            // spurious exit conditions (especially exitCoverB) from firing.
            if (!enterExitInput.useInput)
            {
                ...
                {
                    bool bridgeExitC    = inCover && wasInCover && !goingToCornerPoint &&
                                          !shooterInput.animator.IsInTransition(0) &&
                                          (shooterInput.cc.customAction || isRolling ||
                                           shooterInput.cc.isJumping || shooterInput.cc.ragdolled ||
                                           shooterInput.cc.isDead);
                    bool bridgeForceExit = shooterInput.cc.IsAnimatorTag("ExitCover");

                    if ((bridgeExitC || bridgeForceExit) &&
                        (inCover || (goingToCoverPoint && (remainingDistanceToNextCover > minDistanceToCancelTravel || bridgeForceExit))))
                    {
                        if (goingToCoverPoint) onFinishGoToCoverPoint.Invoke();
                        if (inCover)           onExitCover.Invoke();
                        ExitCover(bridgeForceExit);
                    }
