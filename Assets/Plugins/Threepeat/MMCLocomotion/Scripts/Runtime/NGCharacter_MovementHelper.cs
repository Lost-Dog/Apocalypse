...
namespace Threepeat
{
    ...
    public class NGCharacter_MovementHelper : NGCharacterHelper
    {
        ...
        private float groundedJumpRecoveryStartTime = -1f;
        ...
        protected void CustomUpdate()
        {
            ...
            if (grounded && character.currentState == NGCharacter.CharacterState.JumpingFalling)
            {
                bool jumpOrFallEventActive = mxmAnimator != null && mxmAnimator.IsEventPlaying;
                if (!jumpOrFallEventActive)
                {
                    if (groundedJumpRecoveryStartTime < 0f)
                    {
                        groundedJumpRecoveryStartTime = Time.time;
                    }
                    else if ((Time.time - groundedJumpRecoveryStartTime) > 0.2f)
                    {
                        if (character.debugMode)
                        {
                            Debug.Log("Recovery: grounded while stuck in JumpingFalling with no active event. Forcing Locomotion state.");
                        }

                        character.SetCharacterState(NGCharacter.CharacterState.Locomotion);
                        groundedJumpRecoveryStartTime = -1f;
                    }
                }
                else
                {
                    ...
                    else if ((Time.time - groundedJumpRecoveryStartTime) > 0.6f)
                    {
                        if (character.debugMode)
                        {
                            Debug.Log("Recovery: grounded while JumpingFalling event is still active. Forcing event exit and locomotion restore.");
                        }

                        mxmAnimator.ForceExitEvent();
