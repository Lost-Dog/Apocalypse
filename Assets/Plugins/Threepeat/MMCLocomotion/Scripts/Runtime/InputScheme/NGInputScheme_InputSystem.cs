...
namespace Threepeat
{
    ...
    public class NGInputScheme_InputSystem : NGInputSchemeInputDriven
    {
        ...
        private bool missingMoveActionLogged;
        ...
        public override void Initialize(NGCharacter pCharacter, MxMTrajectoryGenerator pTrajGen)
        {
            ...
            // Ensure we are reading from this scheme's configured actions/map even when a PlayerInput already exists.
            ...
            if (!string.IsNullOrEmpty(inputActionMapActive))
            {
                playerInput.SwitchCurrentActionMap(inputActionMapActive);
            }

            actionMove = ResolveAction(actionNameMovement);
            actionJump = ResolveAction(actionNameJump);
            actionSprint = ResolveAction(actionNameSprint);
            actionCrouch = ResolveAction(actionNameCrouch);
            actionStrafe = ResolveAction(actionNameStrafe);
        ...
        private InputAction ResolveAction(string actionName)
        {
            ...
            if (!string.IsNullOrEmpty(inputActionMapActive))
            {
                actionMap = playerInput.actions.FindActionMap(inputActionMapActive, false);
            }

            if (actionMap != null)
            {
                InputAction mappedAction = actionMap.FindAction(actionName, false);
        ...
        protected override Vector3 GetInputVector()
        {
            if (actionMove == null)
            {
                if (!missingMoveActionLogged)
                {
                    missingMoveActionLogged = true;
                    Debug.LogWarning($"MMLC InputSystem could not find movement action '{actionNameMovement}' in map '{inputActionMapActive}'. Character will not move until action mapping is fixed.");
                }
